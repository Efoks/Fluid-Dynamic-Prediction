...
class EagleDataset(Dataset):
    def __init__(self,
        ...
        with open(os.path.join(cfg.SPLITS_DIR, f"{mode}.txt"), "r") as f:
            for line in f.readlines():
                self.dataloc.append(os.path.normpath(os.path.join(data_path, 'Eagle_dataset', line.strip())))
        ...
        if self.with_cluster:
            assert self.n_cluster in [0, 10, 20, 40, 30], "Number of cluster must be in [0, 10, 20, 30, 40]"
    ...
    def __getitem__(self, item):
        ...
        if self.with_cluster:
            ...
            cluster_path = self.dataloc[item].replace("Eagle_dataset", "Eagle_clusters")
